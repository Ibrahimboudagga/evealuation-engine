...
def test_migration_creates_a_fresh_database(tmp_path):
    ...
    assert {"workspaces", "users", "user_sessions", "workspace_memberships", "provider_connections", "evaluation_templates", "report_shares", "audit_events", "projects", "datasets", "dataset_versions", "evaluation_runs", "evaluation_results", "pairwise_runs", "pairwise_comparisons"} <= set(inspector.get_table_names())
